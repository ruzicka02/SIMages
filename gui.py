...
@ui.page("/select")
def draw_selection(available_dirs: list[Path]):
     @ui.refreshable
     def image_preview():
          ...
          with ui.row().style('display: flex; flex-wrap: wrap; justify-content: left; padding: 0 4px; margin: auto'):
               for img in draw_imgs:
                    with ui.column():
                         ui.label(img.name)
                         with ui.button(on_click=lambda local_img=img: print(local_img)).style('padding: 0px'):
                              print("Draw:", img)
     ...
     dir_toggle = ui.toggle({x: str(x) for x in available_dirs}, on_change=lambda: select_dir(dir_toggle))
